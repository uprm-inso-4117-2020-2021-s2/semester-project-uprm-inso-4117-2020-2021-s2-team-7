...
export default class LevelOfEducationController {
  ...
  public async store({ request, response }: HttpContextContract) {
    ...
    if (!variables) {
      return response.badRequest({ message: 'Specify the required parameters to create a LevelOfEducation' })
    }
    try {
      let createdLevelOfEducation: LevelOfEducation = await LevelOfEducation.create(variables)
      return response.created({ message: 'LevelOfEducation created successfully.', id: createdLevelOfEducation.leid })
    } catch (err) {
      return response.internalServerError({
  ...
  public async update({ request, response, params }: HttpContextContract) {
    ...
    try {
      ...
      if (!levelOfEducation)
        return response.notFound({ message: `Could not find LevelOfEducation with id: ${params.id}` })
